...
*NOTE: This package has moved into the [[https://github.com/emacs-slack/emacs-slack][emacs-slack project]].*

The Org link support now lives there as =slack-org-link.el= (link type,
following, storing, export, completion) and =slack-org-alert.el=
(capturing message alerts as Org TODO headings for the agenda).  Links
keep using the same =emacs-slack:= Org link type, now with Slack
permalinks as their address; the =TEAMID&ROOMID= links stored by this
package are still understood there (the legacy =team - #room= format is
not).  This repository is deprecated and will be archived.
